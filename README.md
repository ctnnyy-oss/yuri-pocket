...
上线后更推荐在“模型与数据”页连接云端口令，再把各供应商的 API Key 保存到服务器模型保险箱。这样手机、平板和电脑只需要同一个云端口令，就能复用同一套模型配置；设备保存过口令后会自动恢复连接。

没有填写密钥时，应用会使用本地兜底回复，方便先验证界面、角色和记忆流程。服务器已配置云端口令和模型密钥时，聊天请求会要求带云端口令，避免公开页面被外部消耗密钥。

当前云端口令是单空间钥匙，适合私用和内测。公开给多用户使用前，需要升级为注册/登录系统，让每个用户拥有独立聊天、记忆、云端数据和模型密钥空间。
