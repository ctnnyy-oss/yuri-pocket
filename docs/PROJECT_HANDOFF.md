...
2. 做模型配置真实回归：至少用当前免费 DeepSeek、一个自定义中转、一个官方接口各测一次。
3. 把聊天消息也纳入更清晰的云端同步策略。
4. 给云端备份再加跨机/异地备份策略。
5. 继续打磨记忆系统：事件账本筛选、云端冲突合并、把聊天透镜反馈扩展成“编辑/归档/删除”完整入口。
6. 做一个新手使用页，告诉妹妹怎么连接云端、怎么保存、怎么恢复、本机保险箱和模型保险箱怎么用。
