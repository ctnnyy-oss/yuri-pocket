...
- 聊天记忆透镜：`src/services/memoryTrace.ts` 把每条助手回复绑定到实际调用过的长期记忆，聊天气泡里可展开查看记忆数量、分组和条目摘要。用户可以在这里直接把误用记忆冷却 7 天，冷却动作会写入事件账本。它回答“这条回复为什么会这样想”，和守护台的全局账本互补。
...
3. 记忆版：候选记忆审核中心、冲突检测、记忆时间线、事件账本、关键词权重、来源证据、版本回滚、使用日志、聊天页记忆调用可视化和误用反馈入口。
