...
- 超过 500 行的文件默认进入“需要继续拆分观察区”。当前 UI 入口、记忆视图、App 编排层、记忆引擎入口、Agent 工具/动作/工具函数/后台平台、ChatPhone 输入面板和 Agent 任务面板都已经拆完；剩余观察项见 `docs/REFACTOR_PROGRESS.md`（目前只有两个大 CSS）。新增能力时按目录边界塞到对应子模块，不要回头堆到 facade。
